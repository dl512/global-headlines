"""
Conversation / CX AI newsletter crawler.
Clears conversation_ai_news.csv, runs optional homepage crawl (usually none), then Google News
from workflow/config/conversation_ai_company_queries.json into the same CSV.
"""
...
def _load_default_company_queries() -> List[Dict[str, Any]]:
    path = os.path.join(_config_dir(), "conversation_ai_company_queries.json")
...
def _load_component_websites_and_queries(
    ...
    with open(config_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    inputs = data.get("components", {}).get("conversation_ai_news", {}).get("crawler", {}).get("inputs", {})
...
async def crawl_conversation_ai_news(
    websites: Optional[List[str]] = None,
    company_configs: Optional[List[Dict[str, Any]]] = None,
) -> List[Dict[str, Any]]:
    """
    Refresh conversation_ai_news.csv: generic crawl for configured sites, then Google News rows.
    """
    websites, company_configs = _load_component_websites_and_queries(websites, company_configs)

    await crawl_news_from_config("conversation_ai_news", websites_override=websites)
    if company_configs:
        await crawl_corporate_news(
            company_configs,
            google_news_simple_csv="conversation_ai_news",
        )
    return []
