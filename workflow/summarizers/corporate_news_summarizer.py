...
async def summarize_corporate_news(
    date: Optional[datetime] = None,
    company_names: Optional[List[str]] = None,
    section_title: Optional[str] = None
) -> str:
    """Summarize corporate news from CSV using LLM
    
    Args:
        date: Optional date to filter news (defaults to today)
        company_names: Optional list of company names to include (if None, includes all)
        section_title: Optional custom section title (defaults to "Corporate News")
    ...
    # Debug: If no matches, show what dates we're looking for vs what we have
    if len(df_filtered) == 0 and len(df) > 0:
        unique_dates = df['Date'].unique()[:10]
        print(f"DEBUG: No matches found. Looking for: {date_formats}")
        print(f"DEBUG: Available dates in CSV: {list(unique_dates)}")
    ...
    if company_names and len(company_names) > 0:
        if 'Company' in df_filtered.columns:
            ...
            print(f"Found {len(df_filtered)} news items matching company names")
        else:
    ...
    prompt = f"""
You are a financial news journalist tasked with creating a concise news-style summary of corporate press releases and announcements.
...
6. **GROUPING**: If multiple announcements are from the same company and related, you can combine them into one sentence with multiple linked phrases, or list them separately if they're distinct events.

Example format:
## {section_title}

Teradyne and MultiLane are [forming a joint venture](https://investors.teradyne.com/news-events/press-releases/detail/432/teradyne-and-multilane-announce-formation-of-joint-venture-multilane-test-products), MultiLane Test Products (MLTP), to accelerate the development of test solutions for high speed data connections. Teradyne will be the majority owner.
Ricursive Intelligence [raised $300M Series A](URL) for AI-driven IC design.
IonQ [plans to acquire SkyWater](URL) for ~$1.8B, creating a "vertically integrated full-stack quantum platform company." IonQ also agreed to acquire R&D specialist Seed Innovations and completed its acquisition of Skyloom.
...
"""
    ...
    try:
        ...
        for item in news_data:
            ...
            if link:
                markdown += f"- **[{headline}]({link})**: {summary} (Company: {company})\n\n"
            else:
                markdown += f"- **{headline}**: {summary} (Company: {company})\n\n"
        return markdown


if __name__ == "__main__":
    summary = asyncio.run(summarize_corporate_news())
    print(summary)
    
    # Save the summary to file
    from common.summary_storage import save_summary
    filepath = save_summary("corporate_news", summary)
    print(f"\n[OK] Summary saved to: {filepath}")
